...
| REQ-AST-02 | QR code generation (opaque token) resolving to a Policy-checked asset page; QR possession alone grants no access | P1 | done (`GET /assets/qr/{token}` re-runs the real `AssetPolicy::view` check; unknown token and unauthorized viewer both 404 identically, never a 403 that would confirm the token's validity) |
...
| REQ-AST-07 | Damage/loss/service/write-off form; service record tracks vendor/due/actual cost/next service; write-off requires authorized approval, history retained | P1 | done (damage/loss/write-off via `ReportAssetIncidentAction`/`DecideAssetIncidentAction`, approval-gated; `ScheduleMaintenanceAction`/`CompleteMaintenanceAction` cover vendor/scheduled/actual-cost/next-service-due tracking, restoring `condition` to `good` only when completing service for an asset actually `under_repair`) |
...
| REQ-AST-10 | Reports: who-holds-what, overdue returns, per-project allocation, service history, lost assets, full asset history | P1 | done |
...
| REQ-NTF-04 | Mobile offline: pre-cached own tasks (bounded), comment/photo draft + queued submission, distinct "saved locally" vs "sent to server" states | P1 | done (task-list pre-cache added: MyDay.vue caches a bounded read-only snapshot on every real fetch and falls back to it — with a visible "last updated" timestamp — when reopened offline with no fresh data yet; verified via a real browser reading the actual IndexedDB store MyDay.vue wrote to. Full cold-start-while-offline access additionally depends on the service worker's own navigation-caching strategy, public/sw.js, not modified in this pass) |
| REQ-NTF-05 | Photo blob local retention when space allows; quota handling with honest (non-false-positive) save confirmation | P1 | done (offlineQueue.ts's enqueue() now catches the real DOMException QuotaExceededError and throws a distinct OfflineQueueQuotaExceededError; MyDay.vue's capture/submit handlers only set a local "queued" status AFTER enqueue() actually succeeds, never optimistically before, so a quota failure can never be shown as a false save confirmation) |
...
| REQ-NTF-09 | IndexedDB-bounded offline queue; resync on foreground/app-open/network-restore; background sync is optional enhancement only, not relied on for iOS closed-app upload | P1 | done (offlineQueue.ts now enforces MAX_PENDING_QUEUE_ITEMS=50 pending items per user; enqueue() refuses a new item past the cap with OfflineQueueFullError, surfaced to the worker as an honest, actionable message rather than a silent accept-then-lose) |
