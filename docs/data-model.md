...
## Implementation notes added by the P0+P1 schema/models/factories pass (2026-09-16)

Everything above this section is unchanged from v1.2. These notes record real, empirically-verified gotchas hit while turning this design into actual migrations against a real Postgres 17 instance — useful for any later agent writing a similar migration, not a change to the data model itself. Full reasoning and the complete list of routine decisions are in `docs/decisions.md`'s "P0+P1 schema pass" section.

- **Self-referencing foreign keys must never be declared inline inside the same `Schema::create()` that defines the table.** Laravel's Postgres grammar compiles `primary`/`foreign` as separate `ALTER TABLE` statements run after the bare `CREATE TABLE`, and the implied `primary` command (from a column's `->primary()` modifier) is queued *after* explicit `->constrained()` foreign-key commands declared earlier in the same closure — so a self-referencing FK's `ALTER TABLE ADD CONSTRAINT` runs before the table's own primary key exists yet, and Postgres rejects it ("no unique constraint matching given keys"). The fix used everywhere in this schema: declare the column as a bare `uuid()` inside `Schema::create()`, then add the actual `->foreign()` in a separate, immediately-following `Schema::table()` call. Affects: `employees.supervisor_employee_id`, `pay_adjustments.reverses_pay_adjustment_id`, `stocktake_lines.recount_of_line_id`, `project_locations.parent_location_id`, `comments.parent_comment_id` (the pre-existing `teams.foreman_employee_id` → `employees` pattern from the Employees domain migration was the template this follows, generalized to true same-table self-references).
- **Every `*_at` timestamp column must use `timestampTz()`/`timestampsTz()`, never `timestamp()`/`timestamps()`, on the `pgsql` grammar.** Laravel's plain `timestamp()` compiles to `timestamp WITHOUT time zone` by default, contradicting this file's own stated convention (`docs/architecture.md` §5: "all `*_at` columns stored in UTC (`timestamptz` in Postgres)"). This was invisible until `attendance_sessions`' overlap-prevention exclusion constraint (`tstzrange(...)`) tried to cast a bare `timestamp` column to `timestamptz` inside an index expression and Postgres refused ("functions in index expression must be marked IMMUTABLE" — that implicit cast depends on the session `timezone` GUC, so it's STABLE, not IMMUTABLE). Every table already migrated before this was caught (Access domain, Employees domain, Devices domain, `attachments`/`notifications`) was retrofitted in place via `2026_09_16_090240_retrofit_timestamp_columns_to_timestamptz.php` rather than editing already-applied migrations.
- **RLS coverage must be added in the SAME phase a table is created**, not assumed to be handled elsewhere: `attachments` and `notifications` (created by the Auth/RBAC pass, before any RLS migration for them existed) were live in the real dev database with real `organization_id` columns but NO row-level-security policy for roughly one migration-authoring session. Closed by `2026_09_16_090230_add_row_level_security_to_p1_business_tables.php`, which also covers every table this pass adds.
