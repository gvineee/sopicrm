...
- Each module's `config/modules/<module>-nav.php` returns an array of entries: label (Georgian), icon, route name, required permission(s), and desktop-menu group (mapped to the section-4 menu groups: მიმოხილვა/ჩემი სამუშაო/პროექტები/დავალებები/თანამშრომლები/დასწრება/ანაზღაურება/ხელსაწყოები/საწყობი/შესყიდვები/ფინანსები/კლიენტები/ხარისხი და უსაფრთხოება/დოკუმენტები/ანგარიშები/პარამეტრები). The mobile bottom navigation is Foundation-owned, limited to five useful destinations and must link only to real routes visible to the signed-in user; notification access is provided from the mobile header.
...
- Device administration supports create, read, and update plus explicit operational status changes. Hard deletion is intentionally excluded: devices may already be referenced by immutable access events, checkpoints, capability snapshots, and sync-command history, so decommissioning must preserve those records rather than cascading away audit evidence.
- **Local-first door decisions:** readers retain the last acknowledged users, credentials, access groups and schedules locally and continue deciding access on the protected site LAN when WAN/Laravel is unavailable. Laravel's desired state and command queue are the control plane, not an online dependency in the physical unlock path. Events buffer at the reader/connector and replay from the durable epoch/native-event checkpoint after recovery. This availability tradeoff is explicit: a revocation requested while a reader is offline cannot take effect instantly at that reader. The UI must show it as desired/pending until that exact reader acknowledges the newer command version; operators must use the site's physical emergency procedure for an immediate offline lockout.
- Normal synchronization is target-scoped reconciliation. It compares each target's latest desired command version with its latest acknowledged version and queues only the divergent target; it never wipes and bulk-reseeds an entire reader during routine recovery.
