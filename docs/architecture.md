...
- Same rule as routes: these are Foundation-owned files. A module needing to register something (a policy binding, an event listener) does so via its own `app/Providers/<Module>ServiceProvider.php`, auto-registered by a single Foundation-owned aggregator the same way seeders are aggregated (glob `app/Providers/*ServiceProvider.php` matching the module-provider naming convention, or explicit registration list maintained by Foundation/Integration — Foundation picks the exact mechanism and documents it in this file once implemented, updating this section).
