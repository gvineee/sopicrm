...
final class SimulatorDeviceAdapter implements AccessControlProviderInterface, DeviceAdapterInterface
{
    ...
    public function generateEventPayload(
        ...
        return [
            ...
            'card_type' => $cardType,
            'card_hex' => $cardHex,
            // Audit A04. This used to be omitted, so App\Domain\Devices\Actions\
            // IngestRawAccessEventAction fell back to its 'device-connector'
            // default and every event generated by the in-app simulator was
            // persisted as though a real reader had produced it — with no
            // column anywhere distinguishing it. The Node-side simulator
            // (services/device-connector/src/adapters/simulator.js) has always
            // set this; the two paths simply disagreed.
            'ingestion_source' => RawAccessEvent::SOURCE_SIMULATOR,
